...
class SpriteManager:
    ...
    def __init__(self, sprite_root: Optional[Union[str, Path]] = None):
        ...
        if sprite_root is None:
            # engine/sprites/sprites.py -> project_root/sprites/
            sprite_root = Path(__file__).resolve().parents[2] / "sprites"
        else:
    ...
    def _calculate_integer_scale(self, original_size: Tuple[int, int], target_size: Optional[Tuple[int, int]]) -> Tuple[int, Tuple[int, int]]:
        ...
        # Downscale (or any shrink) to the exact requested size. Integer-only
        # upscaling is for crisp pixel art; AI / oversized sources must shrink.
    ...
    def is_sprite_fallback(self, sprite: pygame.Surface) -> bool:
        ...
        """
        # pygame-ce Surfaces do not allow arbitrary attributes, so metadata
        # lives in _cache_metadata and is looked up by surface identity.
        for key, cached in self._cache.items():
            if cached is sprite:
                return bool(self._cache_metadata.get(key, False))
        for cached in self._raw_sprite_cache.values():
            if cached is sprite:
                return False
        return True
