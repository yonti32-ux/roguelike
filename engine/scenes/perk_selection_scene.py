...
from .menu_fx import MenuAtmosphere, draw_pulsing_hint, draw_title_glow
...
class PerkSelectionScene:
    ...
    def __init__(self, game: "Game") -> None:
        ...
        self.atmosphere = MenuAtmosphere(self.screen, intensity=0.85, warm=True)
    ...
    def _handle_keydown(self, event: pygame.event.Event) -> Optional[str]:
        ...
        if index is not None and 0 <= index < len(self.current_choices):
            ...
            w, h = self.screen.get_size()
            self.atmosphere.spawn_burst(w // 2, int(h * 0.45), count=18)
    ...
    def draw(self) -> None:
        ...
        header_alpha = int(220 + 20 * abs(math.sin(self.animation_timer * 1.2)))
        header_panel.fill((15, 20, 25, header_alpha))
        self.screen.blit(header_panel, (20, header_y))
        
        # Header border with soft pulse
        border_pulse = 0.6 + 0.4 * abs(math.sin(self.animation_timer * 1.8))
        border_color = (int(80 + 40 * border_pulse), int(100 + 30 * border_pulse), int(120 + 20 * border_pulse))
        pygame.draw.rect(self.screen, border_color, (20, header_y, w - 40, header_height), 2)
        ...
        if queue_count > 0:
            ...
            queue_pulse = 0.75 + 0.25 * abs(math.sin(self.animation_timer * 2.5))
            queue_bg.fill((80, 60, 40, int(180 * queue_pulse)))
            queue_x = w // 2 - queue_surf.get_width() // 2
            queue_y = (owner_y if self.current_owner_name else header_y + 68) + self.font.get_height() + 8
        ...
        for i, perk in enumerate(self.current_choices):
            ...
            # Gentle bob; selected slides slightly forward
            bob = int(3 * math.sin(self.animation_timer * 1.6 + i * 0.8))
            slide = -6 if is_selected else 0
            self._draw_perk_choice(perk, i, y_pos + bob + slide, w, item_height, is_selected)
        ...
        draw_pulsing_hint(
            self.screen,
            self.font_small,
            "Press 1-3 or Enter/Space to select",
            w // 2,
            hint_y,
            self.animation_timer,
        )
        draw_pulsing_hint(
            self.screen,
            self.font_small,
            "Arrow keys: navigate | ESC: cancel all",
            w // 2,
            hint_y + 24,
    ...
    def _draw_perk_choice(self, perk: perk_system.Perk, index: int, y: int, screen_width: int, height: int, is_selected: bool) -> None:
        ...
        if is_selected:
            ...
            glow_alpha = int(70 + 50 * abs(math.sin(self.animation_timer * 3.5)))
