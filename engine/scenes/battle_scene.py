...
from engine.battle.formations import place_enemy_formation
...
from engine.fx.world_atmosphere import BattleAtmosphere
...
class BattleScene:
    ...
    def _init_basic_setup(self) -> None:
        ...
        # Ambient effects
        self._ambient_particles: List[Dict] = []  # kept for compatibility; unused
        self._floating_status_texts: List[Dict] = []  # Floating text when statuses applied
        self.battle_atmosphere = BattleAtmosphere()
        self._screen_size: Tuple[int, int] = (1280, 720)
    ...
    def _init_enemy_units(self, enemies: List[Enemy]) -> None:
        ...
        battle_enemies = list(enemies[:max_enemies])

        # Avoid stacking onto player/companion start tiles (usually left side).
        occupied = {(u.gx, u.gy) for u in self.player_units}
        formation_id, positions = place_enemy_formation(
            battle_enemies,
            grid_width=self.grid_width,
            grid_height=self.grid_height,
            get_terrain=self.terrain_manager.get_terrain,
            occupied=occupied,
        )
        self.enemy_formation_id = formation_id

        enemy_type_list: List[str] = []

        for enemy, (gx, gy) in zip(battle_enemies, positions):
    ...
    def set_status(self, status: BattleStatus) -> None:
        """Update battle outcome and play a one-shot victory/defeat cue."""
    ...
    def update(self, dt: float) -> None:
        ...
        self.battle_atmosphere.update(dt, sw, sh)
    ...
    def draw(self, surface: pygame.Surface, game=None) -> None:
        ...
        # Full dungeon-chamber backdrop behind everything
        if getattr(self, "battle_atmosphere", None) is not None:
            self.battle_atmosphere.draw_scene_backdrop(surface)
        else:
            surface.fill((5, 5, 10))
