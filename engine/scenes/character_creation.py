...
class CharacterCreationScene:
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        self.atmosphere = MenuAtmosphere(screen, intensity=0.95, warm=True)
        # Per-card float offsets for idle bob
        self._card_phases: List[float] = [i * 0.9 for i in range(max(1, len(self.classes)))]
    ...
    def _draw_class_phase(self, w: int, h: int) -> None:
        ...
        # Draw class cards with idle bob / selected lift
        for idx, class_def in enumerate(self.classes):
            is_selected = (idx == self.selected_index)
            card_x = start_x + idx * (card_width + card_spacing)
            phase = self._card_phases[idx] if idx < len(self._card_phases) else idx
            bob = int(4 * math.sin(self.animation_timer * 1.4 + phase))
            lift = int(-10 + 4 * math.sin(self.selection_pulse * 2.2)) if is_selected else 0
            self._draw_class_card(
                class_def,
                card_x,
                card_y + bob + lift,
                card_width,
    ...
    def _draw_class_card(self, class_def, x: int, y: int, width: int, height: int, is_selected: bool) -> None:
        ...
        pulse_alpha = int(40 + 30 * abs(math.sin(self.selection_pulse))) if is_selected else 0
        
        # Card background with border
        border_color = (255, 220, 100) if is_selected else (100, 100, 120)
        border_width = 3 if is_selected else 2
        
        # Draw card shadow (deeper when lifted/selected)
        shadow_offset = 8 if is_selected else 4
        shadow_surf = pygame.Surface((width, height), pygame.SRCALPHA)
        shadow_surf.fill((0, 0, 0, 130 if is_selected else 100))
        self.screen.blit(shadow_surf, (x + shadow_offset, y + shadow_offset))
        
        # Draw glow effect for selected card (behind card)
        if is_selected and pulse_alpha > 0:
            ...
            pygame.draw.rect(glow_surf, glow_color, (0, 0, width + 16, height + 16), 6, border_radius=4)
    ...
    def _draw_name_phase(self, selected, w: int, h: int) -> None:
        ...
        card_y = y
        bob = int(3 * math.sin(self.animation_timer * 1.5))
        card_y += bob
        ...
        pulse = 0.5 + 0.5 * abs(math.sin(self.animation_timer * 2.2))
        border = (int(100 + 60 * pulse), int(100 + 40 * pulse), int(120 + 20 * pulse))
        pygame.draw.rect(self.screen, border, (card_x, card_y, card_width, card_height), 2)
