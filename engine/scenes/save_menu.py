...
from ..utils.save_system import list_saves
from .menu_fx import MenuAtmosphere, draw_title_glow
...
class SaveMenuScene:
    ...
    def __init__(self, screen: pygame.Surface, mode: str = "load") -> None:
        ...
        self.atmosphere = MenuAtmosphere(screen, intensity=0.7, warm=True)
    ...
    def draw(self) -> None:
        ...
        for slot in range(1, 10):
            ...
            # Slot background with soft pulse when selected
            pulse = 0.5 + 0.5 * math.sin(self.animation_timer * 3.5) if is_selected else 0.0
            bg_color = (
                (40 + int(18 * pulse), 40 + int(12 * pulse), 50 + int(8 * pulse))
                if is_selected
                else (25, 25, 30)
            )
            border_color = (255, 230, 160) if is_selected else (100, 100, 100)
            border_w = 3 if is_selected else 2
            
            if is_selected:
                glow = pygame.Surface((slot_width + 8, slot_height + 8), pygame.SRCALPHA)
                pygame.draw.rect(
                    glow,
                    (255, 190, 100, int(35 + 25 * pulse)),
                    glow.get_rect(),
                    border_radius=4,
                )
            ...
            pygame.draw.rect(self.screen, border_color, (x, y, slot_width, slot_height), border_w)
