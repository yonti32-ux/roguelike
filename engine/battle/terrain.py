...
class BattleTerrainManager:
    ...
    def generate_terrain(self) -> None:
        ...
        Player start columns stay clear. On the enemy side we only soft-reserve
        the classic front-line column so the common formation stays readable;
        deeper spawn cells may have cover/obstacles (placement skips blocked
        / hazard tiles).
        """
        ...
        # Soft-reserve classic enemy front column only (not the whole right flank).
        # Rear/advanced cells keep terrain so staggered/spread layouts can use cover.
        enemy_front_col = self.scene.grid_width - BATTLE_ENEMY_START_COL_OFFSET
        for gy in range(self.scene.grid_height):
            reserved_positions.add((enemy_front_col, gy))
