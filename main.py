...
def main() -> None:
    # Match mixer rate to procedural audio BEFORE pygame.init() opens defaults.
    from engine.audio.synth import SAMPLE_RATE
    pygame.mixer.pre_init(SAMPLE_RATE, -16, 2, 512)
    pygame.init()
    pygame.display.set_caption(TITLE)

    # Procedural SFX + music (safe no-op if mixer fails)
    from engine.audio import init_audio, apply_audio_settings
    init_audio()

    # Load resolution + audio settings
